...
let addBlock = function () {
  ...
  infoLang.innerText = 'Смена языка Ctrl + Alt';
...
window.addEventListener('keydown', function (e) {
  for (let i = 0; i < keys.length; i++) {
    ...
    if (e.code === keyClass) {
      if (!rs.classList.contains('hidden') && rs) {
        const value = keys[i].querySelector('.caseDown').textContent;
          // text_input.value += value + 'жопа';
      } 
      else if (!en.classList.contains('hidden') && en) {
        const value = keys[i].querySelector('.caseDown').textContent;
          // text_input.value += value + 'жопа2';
      }
    }


    // if (e.code === keyClass && rs && !rs.classList.contains('hidden') && en && !en.classList.contains('hidden')) {
    //   console.log('jkij')
    //   const value = rs.querySelector('.caseDown').textContent;
...
//     if (e.code === keyClass && e.code !== 'AltLeft' && e.code !== 'AltRight') {
...
//     // Добавить отступ в начало каждой выделенной строки
//     text_input.value = value.substring(0, start) + "\t" + value.substring(end);
//     // Переместить курсор в конец отступа
...
// // функция, которая будет выполняться при нажатии на кнопку клавиатуры
// function pressKey(e) {
...
//   // выполняем соответствующую команду в зависимости от текста кнопки


//   switch (keyText) {
//     case 'Backspace':
//       // удаляем символ перед местом действия курсора
//       text_input.value = text_input.value.slice(0, cursorPos - 1) + text_input.value.slice(cursorPos);
//       // устанавливаем позицию курсора перед удаленным символом
...
// // назначаем обработчик событий на каждую кнопку клавиатуры
